...
class VidEcg_sync:
	...
	def __call__(self, sub, ses, task):
		...
		1 IR video frame = 30 ECG readings'''
...
class thermaldataset(tdata.Dataset):
	...
	def __getitem__(self, index):
		...
		'''IR vid frame rate : 15hz & ECG rate : 450hz, 30 label per video frame'''
		if(start_ts > 0):
			start_frame = start_ts*15
			start_frame_int = math.ceil(start_frame)
			
			# compensate for round-off to integer value
			start_ecg	= int(round((start_frame_int - start_frame)*30))
			print("start_frame", start_frame, start_ecg)
			...
			label_data	  = label_data[start_ecg:(ir_video_data.shape[0]*30)]
		else:
			start_ecg	  = math.ceil(abs(start_ts)*15*30)
			if(start_ecg > label_data.shape[0]*0.75):
				print("getitem, start_ecg> label shape")
				return -1
			label_data	  = label_data[start_ecg:(start_ecg + (ir_video_data.shape[0]*30))]	
		
		#making length of label_data = len(vid)*128
		rem = label_data.shape[0]%30
		label_pad_len = 0
		if(rem != 0):
			div = label_data.shape[0]/30
			label_pad_len = ((int(div)+1)*30) - label_data.shape[0]
		
		label_data = np.pad(label_data, (0, int(label_pad_len)), 'symmetric').reshape((-1, 30))
		#update corresponding label if person moves out of frame
		cur_label = cur_label[:-1].reshape(-1,1)
		...
		raw_label	  = np.cumsum(label_data.flatten(), axis=0).reshape(label_data.shape)	
		mask          = np.ones(raw_label.shape)
		mask[raw_label<0.4] = 0.0 
		#import pdb; pdb.set_trace()
		data_sample = {'data': ir_video_data, 'label' : label_data, 'mask' : mask}
		return data_sample
